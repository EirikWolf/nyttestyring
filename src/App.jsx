...
export default function App(){
  ...
  const effectiveSize=(t)=>{const subs=getSubtasks(t.id);return subs.length>0?subs.reduce((s,sub)=>s+(sub.size||0),0):t.size;};
    ...
    <main id="main" style={{maxWidth:1400,margin:"0 auto",padding:"16px 16px 50px"}}>
      ...
      {role==="employee"&&tab==="submit"&&<div style={{maxWidth:720,margin:"0 auto"}}>
        ...
        <Card style={{marginBottom:12}}><SH>Generell informasjon</SH><div className={s.submitGrid} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          <div style={{gridColumn:"1/-1"}}>
            <label style={{fontSize:11,fontWeight:600,color:C.textSec}}>Behandles det personopplysninger?</label>
            <div style={{display:"flex",gap:12,marginTop:4}}>{[["yes","Ja"],["no","Nei"],["unknown","Vet ikke"]].map(([v,l])=><label key={v} style={{display:"flex",alignItems:"center",gap:4,fontSize:12,cursor:"pointer",color:C.text}}><input type="radio" name="personalData" value={v} checked={form.personalData===v} onChange={e=>setForm(f=>({...f,personalData:e.target.value}))} style={{accentColor:C.primary}}/>{l}</label>)}</div>
            {form.personalData==="yes"&&<div style={{marginTop:6,padding:"6px 10px",background:C.dangerBg,borderRadius:7,fontSize:11,color:C.danger}}>⚠️ Det må gjennomføres en DPIA (personvernkonsekvensvurdering).{config.dpiaLink&&<> <a href={config.dpiaLink} target="_blank" rel="noopener" style={{color:C.danger,textDecoration:"underline"}}>Åpne DPIA-mal →</a></>}{config.dpiaEqsRef&&<span style={{display:"block",fontSize:10,marginTop:2,color:C.textMuted}}>Ref: {config.dpiaEqsRef}</span>}</div>}
          </div>
        </div>
      ...
      {tab==="backlog"&&<div>
        <div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8,marginBottom:10}}><h1 style={{fontSize:20,fontWeight:700,color:C.primary,margin:0}}>Backlog</h1><div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <Btn variant="secondary" onClick={()=>{const rows=sorted.filter(t=>t.reviewed).map(t=>[t.id,t.title,stMap[t.status]?.label,t.priority,(t.tracks||[]).join(";"),composite(t),effectiveSize(t),t.actualHours||0,t.submitterName,t.processOwner||"",t.date,t.sprintId||"",ENTERPRISES.find(e=>e.id===t.enterprise)?.label||t.enterprise||"",SOLUTION_CATEGORIES.find(c=>c.id===t.solutionCategory)?.label||"",t.pilotUnit||"",t.annualTimeSaving||"",t.involvedSystems||""]);exportCsv(["ID","Tittel","Status","Prioritet","Løyper","Score","Story Points","Timer","Innmelder","Prosesseier","Dato","Sprint","Foretak","Løsningskategori","Pilotenhet","Tidsbesparelse (t/år)","Involerte systemer"],rows,"hemit-backlog.csv");notify("CSV eksportert");}} style={{padding:"4px 8px",fontSize:10,background:C.surfaceAlt,color:C.primary}}>📥 CSV</Btn></div></div>
        ...
        :isMobile?<div style={{display:"flex",flexDirection:"column",gap:SP.sm}}>{paged.map(t=>{const sc=composite(t);const pc=prMap[t.priority]?.color||C.textMuted;const st=stMap[t.status];return <Card key={t.id} style={{padding:SP.md,cursor:"pointer",borderLeft:`3px solid ${pc}`,opacity:t.archived?.5:1}} onClick={()=>setModal({type:"review",task:t})}>
          ...
          <div style={{display:"flex",gap:SP.xs,marginTop:SP.xs,flexWrap:"wrap",alignItems:"center"}}>
            ...
            {effectiveSize(t)>0&&<Badge variant="count" size="small">{effectiveSize(t)} SP</Badge>}
        ...
        :<Card style={{padding:0,overflow:"auto"}}><table className={s.backlogTable} style={{width:"100%",borderCollapse:"collapse",fontSize:11}}><thead><tr style={{background:C.surfaceAlt,borderBottom:`2px solid ${C.primary}`}}>{[{h:"",t:""},{h:"#",t:""},{h:"Oppgave",t:""},{h:"Løype",t:""},{h:"Score",t:TIP.Score},{h:"Story Points",t:TIP.SP},{h:"Timer",t:"Faktisk medgått tid i timer"},{h:"Status",t:""},{h:"Tildelt",t:"Teammedlem tildelt oppgaven"},{h:"Sprint",t:"Tildelt sprint"},{h:"",t:""}].map((c,ci)=><th key={ci} title={c.t} style={{padding:"6px 4px",textAlign:"left",fontSize:8,fontWeight:700,color:C.textSec,textTransform:"uppercase",cursor:c.t?"help":"default",borderBottom:c.t?`1px dashed ${C.textMuted}55`:"none"}}>{c.h}</th>)}</tr></thead><tbody>{paged.map((t,i)=>{const sc=composite(t);const open=selectedTask===t.id;const pc=prMap[t.priority]?.color||C.textMuted;return <React.Fragment key={t.id}>
          <tr onClick={()=>setSelectedTask(open?null:t.id)} style={{cursor:"pointer",borderBottom:`1px solid ${C.border}`,background:open?C.surfaceAlt:i%2===0?C.surface:C.surfaceAlt+"80",borderLeft:`3px solid ${pc}`,opacity:t.archived?.5:1}}>
            ...
            <td style={{padding:"6px 3px"}}><span style={{fontSize:14,fontWeight:800,color:sc>200?C.success:sc>100?C.warning:C.danger}}>{sc}</span><span style={{fontSize:8,color:C.textMuted,display:"block"}}>{scoreLabel(sc)}</span></td>
                        <td style={{padding:"6px 3px"}} onClick={e=>e.stopPropagation()}>{isParent(t)?<span style={{fontWeight:700,color:C.accent}} title="Sum av deloppgaver">Σ {effectiveSize(t)}</span>:role==="team"?<SPSel task={t}/>:<span style={{fontWeight:700,color:C.textSec}}>{t.size||"–"}</span>}</td>
      ...
      {tab==="dashboard"&&<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:6}}><h1 style={{fontSize:20,fontWeight:700,color:C.primary,margin:0}}>Dashboard</h1><div style={{display:"flex",gap:4,alignItems:"center"}}>
        ...
        {dashWidgets.includes("goalProgress")&&<Card><h2 style={{fontSize:12,fontWeight:700,color:C.primary,margin:"0 0 10px"}}>Mål-fremgang</h2>{goals.filter(g=>!g.parent).map(main=>{const allIds=[main.id,...goals.filter(g=>g.parent===main.id).map(g=>g.id)];const linked=tasks.filter(t=>!t.archived&&(t.goals||[]).some(g=>allIds.includes(g)));const doneT=linked.filter(t=>t.status==="done"||t.status==="archived");const taskPct=linked.length?Math.round(doneT.length/linked.length*100):0;const tSP=linked.reduce((s,t)=>s+effectiveSize(t),0);const dSP=doneT.reduce((s,t)=>s+effectiveSize(t),0);const spPct=tSP?Math.round(dSP/tSP*100):0;const benefitPct=calcGoalBenefitRealization(linked);return <div key={main.id} style={{marginBottom:12}}>
      ...
      {tab==="portfolio"&&<div>
        ...
        <Card style={{marginBottom:14,padding:0,overflow:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
          ...
          <tbody>{goals.filter(g=>!g.parent).map(main=>{
            ...
            return <tr key={main.id} style={{borderBottom:`1px solid ${C.border}`}}>
              ...
              {TRACKS.map(tr=>{const c=linked.filter(t=>(t.tracks||[]).includes(tr.id));const d=c.filter(t=>t.status==="done"||t.status==="archived").length;const sp=c.reduce((s,t)=>s+effectiveSize(t),0);const dSP=c.filter(t=>t.status==="done"||t.status==="archived").reduce((s,t)=>s+effectiveSize(t),0);const spPct=sp?Math.round(dSP/sp*100):0;return <td key={tr.id} style={{padding:"6px",textAlign:"center",verticalAlign:"middle"}}>
              ...
              <td style={{padding:"6px",textAlign:"center"}}><div style={{fontSize:16,fontWeight:800,color:C.primary}}>{linked.length}</div><div style={{fontSize:8,color:C.textMuted}}>{linked.reduce((s,t)=>s+effectiveSize(t),0)} SP</div></td>
            </tr>;})}</tbody>
        </table></Card>
        ...
        {/* Fordeling per foretak */}
        <Card style={{marginBottom:14}}><h2 style={{fontSize:13,fontWeight:700,color:C.primary,margin:"0 0 10px"}}>🏥 Fordeling per foretak</h2>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
            {ENTERPRISES.map(ent=>{const entTasks=tasks.filter(t=>!t.archived&&t.enterprise===ent.id);const doneTasks=entTasks.filter(t=>t.status==="done"||t.status==="archived");const totalSaving=entTasks.reduce((s,t)=>s+(parseFloat(t.annualTimeSaving)||0),0);const totalCost=entTasks.reduce((s,t)=>s+(parseFloat(t.estimatedCost)||0),0);return entTasks.length>0?<div key={ent.id} style={{padding:12,background:C.surfaceAlt,borderRadius:8,border:`1px solid ${C.border}`}}>
              <div style={{fontSize:12,fontWeight:700,color:C.primary,marginBottom:6}}>{ent.label}</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4,fontSize:10}}>
                <div><span style={{color:C.textMuted}}>Tiltak:</span> <strong>{entTasks.length}</strong></div>
                <div><span style={{color:C.textMuted}}>Ferdig:</span> <strong>{doneTasks.length}</strong></div>
                {totalSaving>0&&<div><span style={{color:C.textMuted}}>Tidsbesparelse:</span> <strong>{totalSaving} t/år</strong></div>}
                {totalCost>0&&<div><span style={{color:C.textMuted}}>Est. kostnad:</span> <strong>{Math.round(totalCost/1000)}k</strong></div>}
              </div>
            </div>:null;})}
          </div>
        </Card>
        ...
        <Card><h2 style={{fontSize:13,fontWeight:700,color:C.primary,margin:"0 0 10px"}}>Porteføljestatus</h2>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:8}}>
            {STATUSES.filter(s=>s.id!=="archived").map(st=>{const c=tasks.filter(t=>!t.archived&&t.status===st.id);const sp=c.reduce((s,t)=>s+effectiveSize(t),0);return <div key={st.id} style={{padding:"10px 12px",background:st.bg,borderRadius:8,border:`1px solid ${st.color}20`}}>
      ...
      {tab==="benefit-report"&&<div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}}>
          ...
          <div style={{display:"flex",gap:4}}>
            <Btn variant="secondary" onClick={()=>{const tks=tasks.filter(t=>!t.archived&&!t.parentId);exportCsvAdvanced(tks,["id","title","status","enterprise","solutionCategory","cost","saving","annualTimeSaving","benefitType","benefitActual","benefitOwner","qualBenefits","quantBenefits"],composite,"hemit-nytterapport.csv");notify("CSV eksportert");}} style={{fontSize:10,padding:"5px 12px",background:C.surfaceAlt,color:C.primary}}>📥 CSV</Btn>
        ...
        {/* KPI-oppsummering */}
        {(()=>{const active=tasks.filter(t=>!t.archived&&!t.parentId);const done=active.filter(t=>t.status==="done");const totalEstCost=active.reduce((s,t)=>s+(parseFloat(t.estimatedCost)||0),0);const totalEstSaving=active.reduce((s,t)=>s+(parseFloat(t.estimatedAnnualSaving)||0),0);const totalRealizedSaving=done.reduce((s,t)=>s+(parseFloat(t.estimatedAnnualSaving)||0),0);const payback=calcAggregatePayback(active);const measurable=active.filter(t=>t.benefitBaseline&&t.benefitTarget&&t.benefitActual);const avgRealization=measurable.length>0?Math.round(measurable.reduce((s,t)=>s+(calcRealizationPct(t.benefitBaseline,t.benefitTarget,t.benefitActual)||0),0)/measurable.length):null;const totalTimeSaving=active.reduce((s,t)=>s+(parseFloat(t.annualTimeSaving)||0),0);const realizedTimeSaving=done.reduce((s,t)=>s+(parseFloat(t.annualTimeSaving)||0),0);return <Card style={{marginBottom:14}}><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:SP.md}}>
          <div style={{padding:12,background:C.surfaceAlt,borderRadius:8,textAlign:"center"}}><div style={{fontSize:9,color:C.textMuted,textTransform:"uppercase",fontWeight:600}}>Est. total kostnad</div><div style={{fontSize:20,fontWeight:800,color:C.primary}}>{totalEstCost>0?`${Math.round(totalEstCost/1000)}k kr`:"—"}</div></div>
          <div style={{padding:12,background:C.surfaceAlt,borderRadius:8,textAlign:"center"}}><div style={{fontSize:9,color:C.textMuted,textTransform:"uppercase",fontWeight:600}}>Est. årlig besparelse</div><div style={{fontSize:20,fontWeight:800,color:C.success}}>{totalEstSaving>0?`${Math.round(totalEstSaving/1000)}k kr`:"—"}</div></div>
          <div style={{padding:12,background:C.surfaceAlt,borderRadius:8,textAlign:"center"}}><div style={{fontSize:9,color:C.textMuted,textTransform:"uppercase",fontWeight:600}}>Realisert besparelse</div><div style={{fontSize:20,fontWeight:800,color:C.accent}}>{totalRealizedSaving>0?`${Math.round(totalRealizedSaving/1000)}k kr`:"—"}</div></div>
          <div style={{padding:12,background:payback!==null?(payback<=1?C.successBg:payback<=3?C.warningBg:C.dangerBg):C.surfaceAlt,borderRadius:8,textAlign:"center"}}><div style={{fontSize:9,color:C.textMuted,textTransform:"uppercase",fontWeight:600}}>Tilbakebetalingstid</div><div style={{fontSize:20,fontWeight:800,color:payback!==null?(payback<=1?C.success:payback<=3?C.warning:C.danger):C.textMuted}}>{payback!==null?`${payback} år`:"—"}</div></div>
          <div style={{padding:12,background:C.surfaceAlt,borderRadius:8,textAlign:"center"}}><div style={{fontSize:9,color:C.textMuted,textTransform:"uppercase",fontWeight:600}}>Tidsbesparelse (t/år)</div><div style={{fontSize:20,fontWeight:800,color:totalTimeSaving>0?C.accent:C.textMuted}}>{totalTimeSaving>0?`${totalTimeSaving}`:"—"}</div>{realizedTimeSaving>0&&<div style={{fontSize:9,color:C.success}}>{realizedTimeSaving} realisert</div>}</div>
          <div style={{padding:12,background:C.surfaceAlt,borderRadius:8,textAlign:"center"}}><div style={{fontSize:9,color:C.textMuted,textTransform:"uppercase",fontWeight:600}}>Nytterealisering</div><div style={{fontSize:20,fontWeight:800,color:avgRealization!==null?(avgRealization>=100?C.success:avgRealization>=50?C.warning:C.danger):C.textMuted}}>{avgRealization!==null?`${avgRealization}%`:"—"}</div><div style={{fontSize:9,color:C.textMuted}}>{measurable.length} målt av {active.length}</div></div>
        </div></Card>;})()}
        {/* Nytte per mål */}
